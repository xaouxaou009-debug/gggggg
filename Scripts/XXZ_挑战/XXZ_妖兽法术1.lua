...
local tbSkill = tbTable:GetSkill("Lua_yaoshou_NormalAttack_1");
			...
			for i = 0, npcs.Count-1,1 do
				...
				if npc.IsDisciple then
					...
					if npc.IsPlayerThing and npc.IsDeath == false and enpc.IsDeath == false then
						npc:ReduceLingDamage((jingjie*qiangdu)/3, skilldef.Element, false, "万雷天引", from);
							
						World.map:StrikePoint(npc.Key , jingjie*(0.2+qiangdu/4) , 0.01)
							
						npc:AddLing(-npc:GetProperty("NpcLingMaxValue")*(0.025+qiangdu/100))
					end
				end
			end
		end	
